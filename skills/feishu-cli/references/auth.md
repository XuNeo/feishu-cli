...
**必须 User Token**：搜索命令（`search docs/messages/apps`）和消息/群聊互动命令（`msg get/list/history/pins/pin/unpin/reaction/delete`、`msg search-chats`、`msg list-chats`、`msg p2p-chat-id`、`user me`、`chat get/update/delete`、`chat member list/add/remove`）通过 `resolveRequiredUserToken` 强制使用 User Token，自动从 token.json 加载。
...
| `im:message.p2p_msg:get_as_user` | 用户身份读取私信（含获取 p2p chat_id） | `msg p2p-chat-id`、`msg list/history`（私信必需） |
| `im:message.group_msg:get_as_user` | 用户身份读取群消息 | `msg list/history`（User Token 读群消息必需） |
| `im:chat:read` | 群聊搜索（User Token） | `msg search-chats`、`msg list-chats` |
...
| `msg p2p-chat-id` | `im:message.p2p_msg:get_as_user`（获取用户间私信 chat_id） |
| `msg list-chats` | `im:chat:read`（枚举所有群聊） |
...
# Device Flow：SSH 远程环境首选，无需配置重定向 URL 白名单
# 推荐 scope（消息读取完整可用集，经过验证）
feishu-cli auth login --method device --scopes "im:message:readonly im:message.p2p_msg:get_as_user im:message.group_msg:get_as_user im:chat:read im:chat:readonly offline_access"
```
...
`--method device` 是 SSH 远程环境的**首选**方案，无需配置重定向 URL 白名单，不依赖本地浏览器回调：

1. 执行 `feishu-cli auth login --method device --scopes "..."`
2. 终端显示用户码和验证链接（含用户码的完整链接），在浏览器中打开完成授权
3. 命令自动轮询等待授权完成，成功后保存 Token

**推荐的消息读取 scope**（经过验证的最小可用集）：
```
im:message:readonly im:message.p2p_msg:get_as_user im:message.group_msg:get_as_user im:chat:read im:chat:readonly offline_access
```

- `im:message.p2p_msg:get_as_user`：读取私信消息，且 `msg p2p-chat-id` 命令必需此 scope
- `im:message.group_msg:get_as_user`：以用户身份读取群消息（Bot 不在群也能读）
- `offline_access`：获取 Refresh Token，避免 2 小时后重新授权

Device Flow 支持 `--scopes` 参数指定 OAuth scope。未指定时默认仅请求 `offline_access`。
